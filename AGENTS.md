...
- Roadmap and references: `docs/roadmap.md`, `docs/references.md`, `docs/code-review-guidelines.md`, `specs/current/maintainability-roadmap.md`, `specs/current/planning-stack-scaffold-plan.md`.
...
## Planning and scaffold workflows

- Project planning is a first-class product surface, not a chat-only prompt pattern. Store accepted briefs, selected tools, stack decisions, scaffold commands, GitHub repo intent, and delivery targets through `/api/plans`.
- Planning work must keep the web UI and `od plan ...` CLI in sync. Stack/tool updates should regenerate the Better-T-Stack scaffold plan from the stored contract instead of duplicating logic in the UI or CLI.
- Tool ids must be role-specific where one provider spans multiple categories. For example, distinguish Cloudflare hosting, Cloudflare data, and Cloudflare Access so downstream agents can connect or defer them independently.
- Better-T-Stack is the default scaffold engine for eligible web projects. When a selected requirement is outside Better-T-Stack's native flags, keep it as an explicit post-scaffold task rather than hiding it in the command.
