...
export interface PlanningExecutionAction {
  id: 'repo-create' | 'scaffold' | 'deploy-runtime' | 'provider-research' | 'project-management' | 'database-materialize' | 'database-migrate' | 'design-materialize';
  label: string;
...
export interface PlanningExecutionArtifact {
  ...
  planId: string;
  runId?: string;
  kind: 'provider-research' | 'section-output' | 'database-draft' | 'database-materialization' | 'database-migration' | 'design-materialization' | 'scaffold-plan' | 'repo-plan' | 'deployment-plan' | 'project-management-plan' | 'tool-check';
  title: string;
  content: string;
  createdAt: number;
}
