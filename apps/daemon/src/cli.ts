...
function printRootHelp() {
  ...
  od plan <tools|list|info|create|update|scaffold|delete> [args]
      Store project planning briefs, stack decisions, Better-T-Stack scaffold
      commands, GitHub repo intent, and delivery targets. Supports --json for
...
async function planDaemonUrl(flags) {
  return cliDaemonUrl(flags);
}
...
async function runPlan(args) {
  ...
  od plan update <id> [--name <title>] [--intent-json <path|->]
                 [--stack-json <path|->] [--tools-json <path|->]
                 [--repo-json <path|->] [--delivery-json <path|->]
...
Common options:
  ...
  switch (sub) {
    ...
    case 'list': {
      ...
      if (plans.length === 0) {
        console.log('No plans. Create one with `od plan create --name "..." --intent-json intent.json`.');
        return;
      }
      for (const plan of plans) {
        console.log(`${plan.id}\t${plan.name}\t${plan.stack?.frontend ?? '-'}\t${plan.stack?.database ?? '-'}\t${plan.repo?.status ?? '-'}`);
      }
      return;
    }
    case 'info': {
      ...
      if (!id) {
        console.error('Usage: od plan info <id>');
        process.exit(2);
      }
    ...
    case 'create': {
      if (!flags.name) {
        console.error('--name <title> is required');
        process.exit(2);
      }
      if (!flags['intent-json']) {
        console.error('--intent-json <path|-> is required');
        process.exit(2);
      }
      const body = readPlanBodyFromFlags(flags, false);
      const resp = await fetch(`${base}/api/plans`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify(body),
      ...
      const data = await resp.json().catch(() => ({}));
      if (!resp.ok) {
        console.error(`POST /api/plans failed: ${resp.status} ${JSON.stringify(data)}`);
        process.exit(1);
      }
      if (flags.json) return process.stdout.write(JSON.stringify(data, null, 2) + '\n');
      console.log(`[plan] created ${data.plan?.id ?? '-'}`);
      console.log(data.plan?.scaffold?.command ?? '');
      return;
    }
    case 'update': {
      const [id] = positionalArgs(rest, PLAN_STRING_FLAGS);
      if (!id) {
        console.error('Usage: od plan update <id> [--name <title>] [--intent-json <path|->] ...');
        process.exit(2);
      }
      const body = readPlanBodyFromFlags(flags, true);
    ...
    case 'scaffold': {
      const [id] = positionalArgs(rest, PLAN_STRING_FLAGS);
      if (!id) {
        console.error('Usage: od plan scaffold <id>');
        process.exit(2);
      }
      const data = await fetchPlan(base, id);
      if (flags.json) return process.stdout.write(JSON.stringify(data.plan?.scaffold ?? {}, null, 2) + '\n');
      console.log(data.plan?.scaffold?.command ?? '');
      const tasks = data.plan?.scaffold?.postScaffoldTasks ?? [];
      if (tasks.length > 0) {
        console.log('');
    ...
    case 'action': {
      ...
      if (!id || !actionId) {
        console.error('Usage: od plan action <id> --action <repo-create|scaffold|deploy-runtime|provider-research|database-materialize|database-migrate|project-management> [--confirmed] [--json]');
        process.exit(2);
      }
    ...
    case 'execute': {
      ...
      const actionId = typeof flags.action === 'string' ? flags.action.trim() : '';
      if (!id || !actionId) {
        console.error('Usage: od plan execute <id> --action <repo-create|scaffold|deploy-runtime|provider-research|database-materialize|database-migrate|project-management> [--confirmed] [--target-dir <path>] [--delivery-target <cloudflare|vercel|coolify|hostinger>] [--project-management-target <github-issues|linear|google-docs>] [--json]');
        process.exit(2);
      }
      const body = {
    ...
    case 'ideas': {
      const [id] = positionalArgs(rest, PLAN_STRING_FLAGS);
      if (!id) {
        console.error('Usage: od plan ideas <id>');
        process.exit(2);
      }
      const resp = await fetch(`${base}/api/plans/${encodeURIComponent(id)}/ideation`);
      if (!resp.ok) return structuredHttpFailure(resp);
      const data = await resp.json();
      if (flags.json) return process.stdout.write(JSON.stringify(data, null, 2) + '\n');
      const sessions = data?.sessions ?? [];
      if (sessions.length === 0) {
        console.log('No brainstorm sessions yet.');
        return;
      }
      for (const session of sessions) {
        console.log(`${session.id}\t${new Date(session.createdAt).toISOString()}\t${session.prompt}`);
      }
      return;
    }
    case 'brainstorm': {
      const [id] = positionalArgs(rest, PLAN_STRING_FLAGS);
      if (!id || (!flags.prompt && !flags['prompt-file'])) {
        console.error('Usage: od plan brainstorm <id> --prompt <text> | --prompt-file <path|->');
        process.exit(2);
      }
      const prompt = flags.prompt ?? readTextFlag(flags['prompt-file'], '--prompt-file');
      const resp = await fetch(`${base}/api/plans/${encodeURIComponent(id)}/ideation`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      ...
      const data = await resp.json().catch(() => ({}));
      if (!resp.ok) {
        console.error(`POST /api/plans/${id}/ideation failed: ${resp.status} ${JSON.stringify(data)}`);
        process.exit(1);
      }
      if (flags.json) return process.stdout.write(JSON.stringify(data, null, 2) + '\n');
      console.log(`[plan] brainstormed ${data.session?.id ?? '-'}`);
      console.log(data.session?.summary ?? '');
      for (const option of data.session?.options ?? []) {
        console.log(`\n${option.title}`);
    ...
    case 'delete': {
      const [id] = positionalArgs(rest, PLAN_STRING_FLAGS);
      if (!id) {
        console.error('Usage: od plan delete <id>');
        process.exit(2);
      }
      const resp = await fetch(`${base}/api/plans/${encodeURIComponent(id)}`, { method: 'DELETE' });
      if (!resp.ok) return structuredHttpFailure(resp);
      console.log(`[plan] deleted ${id}`);
      return;
    }
    default:
      console.error(`unknown subcommand: od plan ${sub}`);
      process.exit(2);
  }
}
...
function readPlanBodyFromFlags(flags, partial) {
  const body = {};
...
function requiredJson(path, flag) {
  const json = safeReadJsonFile(path);
  if (json === null) {
    console.error(`${flag} must point to a valid JSON file, or use - for stdin`);
    process.exit(2);
  }
  return json;
}
