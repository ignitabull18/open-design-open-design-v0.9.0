...
function migrate(db: SqliteDb): void {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS plan_ideation_sessions (
      id TEXT PRIMARY KEY,
...
export function listPlans(db: SqliteDb) {
  return (db
    .prepare(`SELECT ${PLAN_COLS} FROM plans ORDER BY updated_at DESC`)
    .all() as DbRow[])
    .map(normalizePlan);
}

export function getPlan(db: SqliteDb, id: string) {
  const row = db
    .prepare(`SELECT ${PLAN_COLS} FROM plans WHERE id = ?`)
    .get(id) as DbRow | undefined;
...
export function updatePlan(db: SqliteDb, id: string, patch: DbRow) {
...
const PLAN_IDEATION_COLS = `id, plan_id AS planId, prompt, summary,
  options_json AS optionsJson, created_at AS createdAt`;

export function listPlanIdeationSessions(db: SqliteDb, planId: string) {
  return (db
    .prepare(
      `SELECT ${PLAN_IDEATION_COLS}
         FROM plan_ideation_sessions
        WHERE plan_id = ?
        ORDER BY created_at DESC`,
    )
    .all(planId) as DbRow[])
    .map(normalizePlanIdeationSession);
}

export function insertPlanIdeationSession(db: SqliteDb, session: DbRow) {
  db.prepare(
    `INSERT INTO plan_ideation_sessions
       (id, plan_id, prompt, summary, options_json, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
  ).run(
    session.id,
    session.planId,
  ...
  const row = db
    ...
    .get(session.id) as DbRow | undefined;
  return row ? normalizePlanIdeationSession(row) : null;
}

function normalizePlanIdeationSession(row: DbRow) {
  return {
    id: row.id,
    planId: row.planId,
    prompt: row.prompt,
    summary: row.summary,
    options: parseJsonObject(row.optionsJson, []),
    createdAt: Number(row.createdAt),
  };
}

function parseJsonObject(value: unknown, fallback: unknown) {
